        ...
        <div class="card">
            <div class="card-body">
                ...
                <button type="button" id="btnBayar" class="btn btn-gradient-primary w-100 mt-4 d-flex align-items-center justify-content-center" disabled>
                    <span id="btnText">Bayar Sekarang (Midtrans)</span>
...
$(function () {
    ...
    $('#selectVendor').on('change', function () {
        ...
        $('#selectMenu')
            .empty()
            .append('<option value="">-- Pilih Menu --</option>')
            .prop('disabled', true);
    ...
    $('#tabelKeranjang').on('click', 'button[data-index]', function () {
        ...
        if (action === 'decrease') {
            if (listBelanja[index].quantity > 1) {
    ...
    $('#btnBayar').on('click', function () {
        if (isSubmitting || listBelanja.length === 0) {
            return;
        }
        ...
        .fail(function (xhr) {
            const msg = xhr.responseJSON?.message || 'Gagal membuat transaksi checkout.';
            alert(msg);
        })
        .always(function () {
            if (isSubmitting) {
